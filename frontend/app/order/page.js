...
export default function Orders({ searchParams }) {
  ...
  const fetchOrders = async () => {
    try {
      const query = `?filters[customer][user][$eqi]=${user.id}&populate[customer][fields]=user&populate[lineItems]=*&populate[store][fields]=owner,name,meta&populate[payment]=*&pagination[page]=${pageRef.current}&pagination[pageSize]=50&sort=createdAt:desc`;

...
export function Radio({ size = "20", color = "red", ...p }) {
  ...
  return (
    <label
      htmlFor={p.name}
      className={`relative w-[${size}px] h-[${size}px] flex justify-center items-center`}>
      <input
        type="radio"
        id={p.name}
...
export function RadioCard({ Tag, children, cls, ...p }) {
  return (
    <Tag className={`relative w-1/2 md:w-44 h-32 bg-cbg space-y-3 rounded-lg card cd_hr ${cls || ""}`}>
      <input
        type="radio"
        aria-label={p.title || p.name}

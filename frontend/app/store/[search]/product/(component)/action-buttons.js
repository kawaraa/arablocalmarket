...
export default function ActionButtons({}) {
  ...
  const showWarning = () => addMessage({ type: "warning", text: shdCnt.noItemErr[lang], duration: 7 });

  const handleAddToFavorite = () => {
    if (!user) return addMessage({ type: "warning", text: shdCnt.favErr[lang], duration: 10 });
    ...
    addMessage({ type: "success", text: content.addedToFav[lang], duration: 8 });
  };

  const handleAddToCart = () => {
    const items = JSON.parse(window.localStorage.getItem("checkoutItems"));
    if (!items || !items[0]) return showWarning();
    const { storeId, storeName, phone, currency, barcode, name, price, discount, quantity } = items[0];
    const newCart = {
      id: storeId,
      name: storeName,
      phone,
      currency,
      items: [{ barcode, name, price, discount, quantity }],
    };
    ...
    if (cartIndex < 0) carts.push(newCart);
      ...
      if (itemIndex < 0) carts[cartIndex].items.push({ barcode, name, price, discount, quantity });
      else carts[cartIndex].items[itemIndex].quantity = quantity;
    }
    window.localStorage.setItem("carts", JSON.stringify(carts));

    addMessage({ type: "success", text: content.addedToCart[lang], duration: 8 });
  };
  const handleBuy = () => {
    const items = JSON.parse(window.localStorage.getItem("checkoutItems"));
    if (!items || !items[0]) return showWarning();
